...
class _BookAppointmentPageState extends State<BookAppointmentPage> {
  ...
  List<Map<String, dynamic>> vets = [
    {"id": 1, "name": "Dr. Emily Carter", "sub": "Veterinary Clinic", "avatar": "assets/images/vet1.png"},
    {"id": 2, "name": "Dr. Michael Chen", "sub": "Veterinary Clinic", "avatar": "assets/images/vet2.png"},
  ];
  ...
  ImageProvider _petImage(String? path) {
    if (path == null || path.isEmpty) return const AssetImage('assets/icon-1.png');
    if (path.startsWith('/') || path.startsWith('file:')) return FileImage(File(path));
  ...
  Future<void> _save() async {
    ...
    if (petIdSelected == null || vetId == null || dateTimeText == null || (reason ?? "").isEmpty) {
      setState(() { saving = false; error = "Please select pet, vet, date/time and reason"; });
      return;
    }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        backgroundColor: whiteBgColor, foregroundColor: Colors.black, elevation: 0,
      ...
      body: ListView(
        ...
        children: [
          ...
          _box(
            pets.isEmpty
                ? const ListTile(
                    leading: CircleAvatar(backgroundImage: AssetImage('assets/icon-1.png')),
                ...
                : DropdownButtonFormField<int>(
                    ...
                    items: pets.map((p) {
                      ...
                      final sp = (p['species'] ?? '') as String;
                      final ph = (p['photo_path'] ?? '') as String;
                      return DropdownMenuItem<int>(
                        value: id,
                        child: Row(
                          children: [
                            CircleAvatar(radius: 18, backgroundImage: _petImage(ph)),
                            const SizedBox(width: 10),
                            Text(nm, style: const TextStyle(fontWeight: FontWeight.w600)),
                            if (sp.isNotEmpty) ...[
                              const SizedBox(width: 6),
                              Text("($sp)", style: const TextStyle(color: Colors.black54)),
                            ]
                          ],
                        ),
                      );
            ...
            DropdownButtonFormField<int>(
              ...
              items: vets.map((v) {
                return DropdownMenuItem<int>(
                  ...
                  child: Row(
                    children: [
                      CircleAvatar(backgroundImage: AssetImage(v["avatar"] as String)),
                      const SizedBox(width: 10),
                      Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                        Text(v["name"] as String, style: const TextStyle(fontWeight: FontWeight.w600)),
                        Text(v["sub"] as String, style: const TextStyle(color: Colors.black54)),
                      ]),
                    ],
                  ),
                );
          ...
          const SizedBox(height: 8),
          InkWell(onTap: _pickDateTime, child: _pill(dateTimeText ?? "Select Date & Time")),
          ...
          SizedBox(
            ...
            child: ElevatedButton(
              ...
              child: Text(saving ? "Please wait…" : "Confirm",
